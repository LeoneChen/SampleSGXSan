...
static void _harness_ecall_type_char(void) {
  ...
  auto ret = ecall_type_char(__g_harness_eid, val);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_type_char returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_type_int(void) {
  ...
  auto ret = ecall_type_int(__g_harness_eid, val);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_type_int returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_type_float(void) {
  ...
  auto ret = ecall_type_float(__g_harness_eid, val);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_type_float returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_type_double(void) {
  ...
  auto ret = ecall_type_double(__g_harness_eid, val);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_type_double returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_type_size_t(void) {
  ...
  auto ret = ecall_type_size_t(__g_harness_eid, val);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_type_size_t returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_type_wchar_t(void) {
  ...
  auto ret = ecall_type_wchar_t(__g_harness_eid, val);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_type_wchar_t returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_type_struct(void) {
  ...
  auto ret = ecall_type_struct(__g_harness_eid, val);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_type_struct returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_type_enum_union(void) {
  ...
  auto ret = ecall_type_enum_union(__g_harness_eid, val1, val2);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_type_enum_union returned with error code: 0x%X\n",
            ret);
    abort();
  }
}
static void _harness_ecall_pointer_user_check(void) {
  ...
  auto ret = ecall_pointer_user_check(__g_harness_eid, &_fuzz_ret, val, sz);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_pointer_user_check returned with error code: 0x%X\n",
            ret);
    abort();
  }
}
static void _harness_ecall_pointer_in(void) {
  ...
  auto ret = ecall_pointer_in(__g_harness_eid, val);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_pointer_in returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_pointer_out(void) {
  ...
  auto ret = ecall_pointer_out(__g_harness_eid, val);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_pointer_out returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_pointer_in_out(void) {
  ...
  auto ret = ecall_pointer_in_out(__g_harness_eid, val);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_pointer_in_out returned with error code: 0x%X\n",
            ret);
    abort();
  }
}
static void _harness_ecall_pointer_string(void) {
  ...
  auto ret = ecall_pointer_string(__g_harness_eid, str);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_pointer_string returned with error code: 0x%X\n",
            ret);
    abort();
  }
}
static void _harness_ecall_pointer_string_const(void) {
  ...
  g_alloc_mgr.push_back((uint8_t *)str);
  g_fdp->ConsumeData(str, str_strlen * sizeof(char));
  auto ret = ecall_pointer_string_const(__g_harness_eid, str);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr,
            "ecall_pointer_string_const returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_pointer_size(void) {
  ...
  auto ret = ecall_pointer_size(__g_harness_eid, ptr, len);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_pointer_size returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_pointer_count(void) {
  ...
  auto ret = ecall_pointer_count(__g_harness_eid, arr, cnt);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_pointer_count returned with error code: 0x%X\n",
            ret);
    abort();
  }
}
static void _harness_ecall_pointer_isptr_readonly(void) {
  ...
  auto ret = ecall_pointer_isptr_readonly(__g_harness_eid, buf, len);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr,
            "ecall_pointer_isptr_readonly returned with error code: 0x%X\n",
            ret);
    abort();
  }
}
static void _harness_ocall_pointer_attr(void) {
  auto ret = ocall_pointer_attr(__g_harness_eid);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ocall_pointer_attr returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_array_user_check(void) {
  ...
  auto ret = ecall_array_user_check(__g_harness_eid, arr);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_array_user_check returned with error code: 0x%X\n",
            ret);
    abort();
  }
}
static void _harness_ecall_array_in(void) {
  ...
  auto ret = ecall_array_in(__g_harness_eid, arr);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_array_in returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_array_out(void) {
  int arr[4];
  auto ret = ecall_array_out(__g_harness_eid, arr);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_array_out returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_array_in_out(void) {
  ...
  auto ret = ecall_array_in_out(__g_harness_eid, arr);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_array_in_out returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_array_isary(void) {
  ...
  auto ret = ecall_array_isary(__g_harness_eid, arr);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_array_isary returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_function_public(void) {
  auto ret = ecall_function_public(__g_harness_eid);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_function_public returned with error code: 0x%X\n",
            ret);
    abort();
  }
}
static void _harness_ecall_function_private(void) {
  int _fuzz_ret;
  auto ret = ecall_function_private(__g_harness_eid, &_fuzz_ret);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_function_private returned with error code: 0x%X\n",
            ret);
    abort();
  }
}
static void _harness_ecall_malloc_free(void) {
  auto ret = ecall_malloc_free(__g_harness_eid);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_malloc_free returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_sgx_cpuid(void) {
  ...
  auto ret = ecall_sgx_cpuid(__g_harness_eid, cpuinfo, leaf);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_sgx_cpuid returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_exception(void) {
  auto ret = ecall_exception(__g_harness_eid);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_exception returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_map(void) {
  auto ret = ecall_map(__g_harness_eid);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_map returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_increase_counter(void) {
  size_t _fuzz_ret;
  auto ret = ecall_increase_counter(__g_harness_eid, &_fuzz_ret);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_increase_counter returned with error code: 0x%X\n",
            ret);
    abort();
  }
}
static void _harness_ecall_producer(void) {
  auto ret = ecall_producer(__g_harness_eid);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_producer returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_consumer(void) {
  auto ret = ecall_consumer(__g_harness_eid);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_consumer returned with error code: 0x%X\n", ret);
    abort();
  }
}
static void _harness_ecall_test_string_hooks(void) {
  auto ret = ecall_test_string_hooks(__g_harness_eid);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr, "ecall_test_string_hooks returned with error code: 0x%X\n",
            ret);
    abort();
  }
}
static void _harness_ecall_test_string_hooks_oob(void) {
  ...
  auto ret = ecall_test_string_hooks_oob(__g_harness_eid, test_id);
  if (ret != SGX_SUCCESS && ret != SGX_ERROR_INVALID_PARAMETER &&
      ret != SGX_ERROR_ECALL_NOT_ALLOWED) {
    fprintf(stderr,
            "ecall_test_string_hooks_oob returned with error code: 0x%X\n",
            ret);
    abort();
  }
}
